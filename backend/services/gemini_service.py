...
class GeminiService:
    """Service for analyzing Figma designs using Google Gemini API"""
        ...
        for idx, child in enumerate(document['children']):
            if child.get('type') == 'CANVAS':
                ...
                if 'children' in child:
                    for component in child['children'][:10]:  # Limit to first 10
...
"""
                ...
                try:
                    response = self.model.generate_content(prompt)
                    page_analysis = json.loads(response.text.strip())
...
"""
        ...
        try:
            response = self.model.generate_content(prompt)
            feature_specs = json.loads(response.text.strip())
...
"""
        ...
        try:
            response = self.model.generate_content(prompt)
            implementation_guide = json.loads(response.text.strip())
